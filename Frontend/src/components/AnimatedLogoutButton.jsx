 ...
 * 
 * Interactive logout button with animated figure that walks out and falls
 * when clicked. Visual feedback shows a character walking through a door
 * and falling off screen when logout is triggered.
 * 
 * Features:
 * - Animated figure with walking and falling states
 * - Door opening animation on hover and click
 * - Responsive button with dark/light theme support
 * - Smooth transitions and easing animations
 * - Ready to integrate logout functionality
 * 
 * Props:
 ...
 * - CSS module: AnimatedLogoutButton.css
 */

import { useEffect, useRef, useState } from "react";
import "../styles/components/AnimatedLogoutButton.css";

function AnimatedLogoutButton({ onLogout, theme = "dark" }) {
  const buttonRef = useRef(null);
  const [isAnimating, setIsAnimating] = useState(false);

  // Define animation states for the button
  ...
  // Update button state with CSS variables
  ...
  const handleMouseEnter = () => {
    if (buttonRef.current && !isAnimating) {
      if (!buttonRef.current.classList.contains("clicked")) {
  ...
  return (
    <button
      ref={buttonRef}
      className={`logoutButton logoutButton--${theme}`}
